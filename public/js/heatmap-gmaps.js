	...
	if(this.latlngs.length > 0){
		...
		while(len--){
			...
			var screenPixel = new google.maps.Point(
			        divPixel.x - leftX,
			        divPixel.y - topY);
			var roundedPoint = this.pixelTransform(screenPixel, true);
			d.data.push({ 
			    x: roundedPoint.x,
			    y: roundedPoint.y,
...
HeatmapOverlay.prototype.pixelTransform = function(p,a){
...
HeatmapOverlay.prototype.setDataSet = function(data){
	...
	while(dlen--){	
		...
		var point = this.pixelTransform(projection.fromLatLngToDivPixel(latlng), false);
		//if (!flag) { console.log(projection.fromLatLngToDivPixel(latlng).x + " - " + projection.fromLatLngToDivPixel(latlng).y + " | " + d[dlen].lat + " - " + d[dlen].lng); ++flag; }

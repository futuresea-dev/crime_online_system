 ...
 */ 
ol.layer.Base.Heatmap = new ol.layer.Base( {
	...
	initialize: function(name, map, mLayer, hmoptions, options){
		...
		heatdiv.style.cssText = "position:absolute;width:"+map.getSize[1]+"px;height:"+map.getSize[0]+"px;";
	...
	setDataSet: function(obj){
		...
		while(dlen--){
			var lonlat = ol.proj.fromLonLat([dataset[dlen].lon, dataset[dlen].lat]);
			var pixel = this.roundPixels(this.mapLayer.getPixelFromCoordinate(lonlat));
			if(pixel)
	...
	CLASS_NAME: "ol.layer.Base.Heatmap"
});
